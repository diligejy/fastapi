...
app = FastAPI(
    title="Jinyoung",
    description="Jinyoung does not only code but also helps you fix things. **See what's in stock!**",
    version="0.1.0",
)
...
Selection = dict[
    str, str or int or float or Category or None
...
def query_item_by_parameters(
    ...
    category: Category or None = None,
) -> dict[str, Selection or list[Item]]:
...
def update(
    ...
    price: float or None = Query(
        title="Price",
        description="New price of the item in Euro.",
        default=None,
        gt=0.0,
    ),
    count: int or None = Query(
        title="Count",
